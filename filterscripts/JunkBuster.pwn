...
PRIVATE: JB::MutePlayer (playerid, time, reason [])//Time is seconds.
{
	if (IsPlayerConnected (playerid) && time)
	{
		...
		JB::SendFormattedMessageToAll (JB_RED, "{F81414}(BE-AC) {FFFFFF}Muting player '%s' (%d) for %d second(s). Reason: %s", JB::PlayerInfo [playerid][JB::pName], playerid, time, reason);
		return 1;
	}
	return 0;
}
...
PRIVATE: JB::Kick (playerid, reason [])
{
	if (IsPlayerConnected (playerid) && !IsPlayerNPC (playerid))
	{
		...
		JB::SendFormattedMessageToAll (JB_RED, "{F81414}(BE-AC) {FFFFFF}Kicking player '%s'. Reason: %s", JB::PlayerInfo [playerid][JB::pName], reason);
...
PRIVATE: JB::Ban (playerid, reason [])
{
	if (IsPlayerConnected (playerid) && !IsPlayerNPC (playerid))
	{
		...
		TogglePlayerControllable (playerid, false);
		SetCameraBehindPlayer (playerid);
		JB::SendFormattedMessageToAll (JB_RED, "{F81414}(BE-AC) {FFFFFF}Banning player '%s'. Reason: %s", JB::PlayerInfo [playerid][JB::pName], reason);
...
PRIVATE: DisplayKickBanText (playerid, reason [], bool: ban)
{
	if (IsPlayerConnected (playerid))
	{
		...
		format (string, sizeof (string), "~y~Name:~n~~w~%s~n~~y~Datum:~n~~w~%s~n~~y~Vrijeme:~n~~w~%s~n~~y~IP:~n~~w~%s~n~~y~Razlog:~n~~w~%s", JB::PlayerInfo [playerid][JB::pName], JB::GetDate (), JB::GetTime (),
		...
		format (string, sizeof (string), "~g~Ako ste greskom %s, pritisnite ~b~F8~g~ da napravite screenshot i postavite na forum.", ban ? ("banovani") : ("kikovani"));
...
PUBLIC: CheckText (playerid, text []) //return 1: something forbidden found, return 0: everything OK!
{
	...
	{
		SendClientMessage (playerid, JB_RED, "{F81414}(BE-AC) {FFFFFF}Iskljucite CAPSLOCK");
		return 1;
	}
	...
	{
		SendClientMessage (playerid, JB_RED, "{F81414}(BE-AC) {FFFFFF}Upisali ste zabranjenu rijec");
		return 1;
	}
	return 0;
}
...
PRIVATE: JunkBusterReport (playerid, report [], details [])
{
	if (IsPlayerConnected (playerid))
	{
		...
		format (string, sizeof (string), "{F81414}(BE-AC) {FFFFFF}Suspected player with %s: %s (%d, %s), %s.", report, JB::PlayerInfo [playerid][JB::pName], playerid, JB::PlayerInfo [playerid][JB::pIp], details);
		ForEachPlayer (i)
		if (JB::IsPlayerAdmin (i) && i != playerid)
		SendClientMessage (i, JB_RED, string);
        {
        	format (string, sizeof (string), "{F81414}(BE-AC) {FFFFFF}Suspected player %s (Vehicle Troll)", JB::PlayerInfo[playerid][JB::pName]);
	 	    JB::Kick (playerid, string);
        }
			...
			if (!JB::IsPlayerAdmin (i) && IsPlayerInValidState (i) && !DOB::GetBit (JB::AntiBugKilled, i)) //!JB::PlayerInfo [i][JB::pAntiBugKilled])
			{
			    ...
			    if (JB::Variables [JOYPAD] && !JB::PlayerInfo [i][JB::pFreezeTime])
				{
				    ...
				    if((updown != 128 && updown != 0 && updown != -128) || (leftright != 128 && leftright != 0 && leftright != -128))
				    {
						...
						else
						{
						    ...
						    SetSyncTime (i, SYNC_TYPE_VEHICLE);
							TogglePlayerControllable (i, false);
							SendClientMessage (i, JB_RED, "{F81414}(BE-AC) {FFFFFF}Please stop using a joypad/gamepad. Use your keyboard and mouse.");
						}
				    }
				}
				...
				if (JB::Variables [MIN_FPS])
				{
					...
					if (var < JB::Variables [MIN_FPS])
					{
						...
						{
							JB::SendFormattedMessage (i, JB_RED, "{F81414}(BE-AC) {FFFFFF}Please fix your framerate (FPS) or you will get kicked! (Min %d, your FPS: %d)", JB::Variables [MIN_FPS], var);
							SendClientMessage (i, JB_RED, "{F81414}(BE-AC) {FFFFFF}Pressing F7 once may help to fix it. (Removes the outlines of the letters in chat window.)");
						}
					}
				}
		...
		for (new i = 0; i < TempBanCount; ++i)
		{
			if (!strcmp (TempBanInfo [i][tbName], JB::PlayerInfo [playerid][JB::pName], false))
			{
				if (TempBanInfo [i][tbTime] > time)
				{
					...
					JB::SendFormattedMessage (playerid, JB_RED, "{F81414}(BE-AC) {FFFFFF}You are temporary for %d day(s), %d hour(s), %d minute(s) and %d second(s)!", days, hours, minutes, seconds);
		    ...
		    if (JB::PlayerInfo [killerid][JB::pCurrentState] == PLAYER_STATE_DRIVER)
		    {
				if (JB::Variables [DRIVE_BY] >= 2 && (reason == WEAPON_UZI || reason == WEAPON_MP5 || reason == WEAPON_TEC9 || reason == 50)) // 50 = helicopter blades.
				{
					...
					else
					{
						...
						SendClientMessage (killerid, JB_RED, "{F81414}(BE-AC) {FFFFFF}You have been killed for drive-by!");
					}
				}
			 	...
			 	if (JB::Variables [ARMED_VEHICLES])
			 	{
					...
					if (IsArmedVehicle (vehicleid))
					{
					    if (++JB::Warnings [killerid]{ARMED_VEHICLES} >= JB::Variables [ARMED_VEHICLES])
						{
							...
							JB::SendFormattedMessage (killerid, JB_RED, "{F81414}(BE-AC) {FFFFFF}Killing with a %s is not allowed! Stop it! (Warning %d/%d)", JB::GetVehicleName (vehicleid), JB::Warnings [killerid]{ARMED_VEHICLES}, JB::Variables [ARMED_VEHICLES]);
					}
			 	}
			}
			...
			if (JB::Variables [SPAWNKILL])
			{
				if (JB::PlayerInfo [playerid][JB::pSpawnKillProtected])
				{
					if (++JB::Warnings [killerid]{SPAWNKILL} >= JB::Variables [SPAWNKILL])
						...
						JB::SendFormattedMessage (killerid, JB_RED, "{F81414}(BE-AC) {FFFFFF}Do not spawnkill! (Warning %d/%d)", JB::Warnings [killerid]{SPAWNKILL}, JB::Variables [SPAWNKILL]);
				}
		 	}
		}
...
CALLBACK: OnPlayerConnect (playerid)
{
	...
	if (!IsPlayerNPC (playerid))
	{
	        ...
	        if (JB::Variables [BLACKLIST])
	        {
		        ...
		        if (banned)
		        {
		            ...
		            {
		                SendClientMessage (playerid, JB_RED, "{F81414}(BE-AC) {FFFFFF}You are banned from this server!");
						JB::LogEx ("%s (%s) has been banned for Ban evading.", JB::PlayerInfo [playerid][JB::pName], JB::PlayerInfo [playerid][JB::pIp]);
					...
					else if (t < unbantime)
					{
					    ...
					    JB::SendFormattedMessageToAll (JB_RED, "{F81414}(BE-AC) {FFFFFF}You are temporary for %d day(s), %d hour(s), %d minute(s) and %d second(s)!", days, hours, minutes, seconds);
	...
	{
		//SendClientMessage (playerid, JB_GREEN_BLUE, "{F81414}(BE-AC) {FFFFFF} Ovaj!");
...
CALLBACK: OnPlayerText (playerid, text [])
{
	...
	{
		JB::SendFormattedMessage (playerid, JB_RED, "{F81414}(BE-AC) {FFFFFF}You are not allowed to chat for %d second(s) because you have been muted!", JB::PlayerInfo [playerid][JB::pMuted]);
		return 0;
	}
...
COMMAND:jbchrome (playerid, params [])
{
	...
	if (JunkBusterChrome (id, JB::PlayerInfo [playerid][JB::pName]))
		JB::SendFormattedMessage (playerid, JB_GREEN, "{F81414}(BE-AC) {FFFFFF}JunkBuster Chrome has successfully collected data of player '%s'!", JB::PlayerInfo  [id][JB::pName]);
	else
	    JB::SendFormattedMessage (playerid, JB_RED, "{F81414}(BE-AC) {FFFFFF}JunkBuster Chrome could not colect data of player '%s'!", params);
	return 1;
}
...
COMMAND:jbsethomepage (playerid, params [])
{
    ...
    DOF2::SetString (CONFIG_FILE, "Homepage", JB::Homepage, "Misc");
    JB::SendFormattedMessage (playerid, JB_GREEN, "{F81414}(BE-AC) {FFFFFF}Homepage has been set to '%s'!", params);
	return 1;
}
...
COMMAND:blackadd (playerid, params [])
{
	...
	if (!IsPlayerConnected (id))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "{F81414}(BE-AC) {FFFFFF}Player '%s' has successfully been added to blacklist!", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "{F81414}(BE-AC) {FFFFFF}Could not add player '%s' to blacklist!", params);
	}
	else
	{
		if (id != playerid)
		{
		        ...
		        {
		            JB::SendFormattedMessage (playerid, JB_GREEN, "{F81414}(BE-AC) {FFFFFF}Player '%s' has successfully been added to blacklist!", JB::PlayerInfo [id][JB::pName]);
				...
				if (AddPlayerToBlacklist (id))
					...
					JB::SendFormattedMessage (playerid, JB_RED, "{F81414}(BE-AC) {FFFFFF}Could not add player '%s' to blacklist!", JB::PlayerInfo [id][JB::pName]);
		}
	}
	return 1;
}
...
COMMAND:blackdel (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "{F81414}(BE-AC) {FFFFFF}Player '%s' has successfully been removed from blacklist!", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "{F81414}(BE-AC) {FFFFFF}Could not remove player '%s' from blacklist!", params);
			return 1;
	}
...
COMMAND:whiteadd (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "{F81414}(BE-AC) {FFFFFF}Player '%s' has successfully been added to whitelist!", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "{F81414}(BE-AC) {FFFFFF}Could not add player '%s' to whitelist!", params);
			return 1;
	}
...
COMMAND:whitedel (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "{F81414}(BE-AC) {FFFFFF}Player '%s' has successfully been removed from whitelist!", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "{F81414}(BE-AC) {FFFFFF}Could not remove player '%s' from whitelist!", params);
			return 1;
	}
...
COMMAND:jbsetvar (playerid, params [])
{
	...
	{
	    JB::SendFormattedMessage (playerid, JB_GREEN, "{F81414}(BE-AC) {FFFFFF}JunkBuster variable '%s' has successfully been set to %d.", var, value);
...
COMMAND:tunban (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "{F81414}(BE-AC) {FFFFFF}Temporary ban of player '%s' has successfully been deleted.", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "{F81414}(BE-AC) {FFFFFF}Could not delete temporary ban of player '%s'!", params);
			return 1;
	}
...
COMMAND:banip (playerid, params [])
{
	if (!IsPlayerAdmin (playerid))
	    ...
	    SendClientMessage (playerid, JB_RED, "{F81414}(BE-AC) {FFFFFF}Sorry, this command is currently not available when using JunkBuster database.");
	    return 1;
	#else
		if (!isnull (params))
		{
			...
			{
				if (BanIp (params))
					JB::SendFormattedMessage (playerid, JB_GREEN, "{F81414}(BE-AC) {FFFFFF}IP %s has successfully been banned!", params);
				else
					JB::SendFormattedMessage (playerid, JB_RED, "{F81414}(BE-AC) {FFFFFF}Could not ban IP %s!", params);
				return 1;
			}
		}
...
COMMAND:unbanip (playerid, params [])
{
	if (!IsPlayerAdmin (playerid))
	    ...
	    if (JBDB::UnbanIp (params))
			JB::SendFormattedMessage (playerid, JB_GREEN, "{F81414}(BE-AC) {FFFFFF}IP %s has successfully been unbanned!", params);
		else
			JB::SendFormattedMessage (playerid, JB_RED, "{F81414}(BE-AC) {FFFFFF}Could not unban IP %s!", params);
		return 1;
	#else
		if (!isnull (params))
		{
				...
				case 24, 25, 27, 29, 30, 31, 33, 34: // Deagle, Shotung, SPAS12, MP5, AK47, M4, Rifle, Sniper Rifle
				{
					...
					else if (((oldkeys & KEY_FIRE) || (oldkeys == KEY_FIRE)) && ((newkeys & KEY_CROUCH) || (newkeys == KEY_CROUCH)) && (GetTickCount () - JB::PlayerInfo [playerid][JB::pFired]) < 750)
					{
						if (++JB::Warnings [playerid]{CBUG} & 1 == 0)
							SendClientMessage (playerid, JB_RED, "{F81414}(BE-AC) {FFFFFF}Please stop performing the C-Bug or you will get kicked/banned.");
	...
	if (1 <= JB::Variables [DRIVE_BY] <= 2 && PRESSED (KEY_FIRE) && (HOLDING (KEY_LOOK_RIGHT) || HOLDING (KEY_LOOK_LEFT)))
	{
		if (!IsPlayerInPlane (playerid) && GetVehicleModel (GetPlayerVehicleID (playerid)) != 432)//Ignore planes and rhinos.
		{
			...
			if (weaponid != 0 && ammo != 0)//Check if player has got an SMG. Don't punish innoncent players.
			{
				...
				SendClientMessage (playerid, JB_RED, "{F81414}(BE-AC) {FFFFFF}Please stop performing drive-by!");
				TogglePlayerControllable (playerid, false);
			...
			case DIALOG_CFG:
			{
				if (response)
				{
					switch (listitem)
					{
						...
						{
							ConfigJunkBuster ();
							SendClientMessage (playerid, JB_GREEN, "{F81414}(BE-AC) {FFFFFF}Configuration has been loaded from file.");
							ShowPlayerConfigDialog (playerid);
						}
						...
						{
							SaveJunkBusterVars ();
							SendClientMessage (playerid, JB_GREEN, "{F81414}(BE-AC) {FFFFFF}Configuration has been saved to file.");
							ShowPlayerConfigDialog (playerid);
						}
						...
						{
							JB::Variables = JB::DefaultVariables;
							SendClientMessage (playerid, JB_GREEN, "{F81414}(BE-AC) {FFFFFF}Default configuration has been loaded.");
			...
			case DIALOG_SETVAR .. (DIALOG_SETVAR + MAX_JB_VARIABLES - 1):
			{
				if (response)
				{
					...
					else
					{
						...
						JB::SendFormattedMessage (playerid, JB_GREEN, "{F81414}(BE-AC) {FFFFFF}JunkBuster variable '%s' has successfully been set to %d.", JB::VariableNames [var], JB::Variables [var]);
...
#if !defined I_AM_NO_RETARD
	#error You're a retard. Read the bloody README as you are told!
#endif
